...
angular.module('ng-validation', []).factory('ngValidation', ['ngValidationRules', 'ngValidationScenarios', function(ngRules, ngScenarios) {
  ...
  Validation.prototype.validate = function(el, args, e) {
    ...
    //allow for custom validation flags, this allows you to specify an attribute on the
    //input field validator="" so you can call the specific validator
    if(el.hasAttribute("validator") && this.opts.validators[el.getAttribute("validator")]) {
      // If it passes
      if(this.opts.validators[el.getAttribute("validator")].call(this, el, args, e)) {
    ...
    else if(this.validators[validator] && this.validators[validator].call(this, el)) {
   ...
   */
  API.add_rule = API.addRule = function(name, rule) {
    var bad = "@!#$%^&*(){}[]/-+=|\"' ";
    var isBad = [];
