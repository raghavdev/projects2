...
ngDataApp.service('loader', ['ngData', function(ngData) {
  ...
  var Loader = function() {
    ...
    this.minLoaderDisplayTime = ngData.loader.minLoaderDisplayTime || 1000;
  ...
  /**
   * RUN
   * @param  {Object} model WHen loader is called with a model, that model gets added to a bank or queue.
   *                        The models get removed from the queue via the Model layer when loaded is called with the same model.
   * @return {[type]}       [description]
   */
  Loader.prototype.run = function(model) {
    ...
    if(this.enabeled) {
      if(model) {
        ...
        if(model.loader) {
          if(model.loader.content) {
            content = model.loader.content;
          }
          if(model.loader.loader) {
            loaderElem = model.loader.loader;
          }
        }
      }
  ...
  Loader.prototype.loaded = function(model) {
    ...
    if(model) {
      for(var i=0; i<this.queue.length; i++) {
        if(model.name !== this.queue[i].name) {
