 ...
 */
app.directive('tagimacator', function($http, $parse, $rootElement, $q) {
  return {
    restrict: 'EA',
      ...
      $scope.loadTagimacatedData = function() {
        ...
        if($scope.tagimacatorData) {
          if($scope.tagimacatorData.constructor === Function) {
            ...
            if(!$scope.tagimacatorData.constructor === Array && !$scope.tagimacatorData.length || 
              $scope.tagimacatorData.constructor === Object && !Object.keys($scope.tagimacatorData).length) {
              $scope.watch('tagimacatorData', function(newVal, oldVal) {
                def.resolve(newVal);
              })
            } else {
